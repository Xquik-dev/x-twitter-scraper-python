...
NUMERIC_CASES: list[tuple[str | bytes | int | float, datetime | Type[Exception]]] = [
...
DATE_CASES = [(value, result.date() if isinstance(result, datetime) else result) for value, result in NUMERIC_CASES] + [
    # Valid inputs
...
@pytest.mark.parametrize(
    "parser,value,result",
    [(parse_date, value, result) for value, result in DATE_CASES]
    + [(parse_datetime, value, result) for value, result in DATETIME_CASES],
)
def test_parsing(
    parser: Callable[[str | bytes | int | float], date],
    value: str | bytes | int | float,
    result: date | Type[Exception],
) -> None:
    if isinstance(result, type):
        with pytest.raises(result):
            parser(value)
    else:
        assert parser(value) == result
